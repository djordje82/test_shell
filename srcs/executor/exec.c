...
static void	process_pipe_loop(t_command *current, t_shell *shell, 
			int prev_pipe[2], pid_t *last_pid)
{
	while (current)
	{
...
static void	restore_terminal_settings(int original_stdin_fd, 
		...
		struct sigaction *sa_old_int, struct sigaction *sa_old_quit)
{
	...
	process_pipe_loop(current, shell, prev_pipe, &last_pid);
	wait_for_children(last_pid);
	restore_terminal_settings(original_stdin_fd, &orig_termios);
